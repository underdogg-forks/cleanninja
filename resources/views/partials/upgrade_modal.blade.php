...
<div class="col-md-12 text-center">
  ...
  @if (Auth::user()->account->plan->hasActivePromo())
    <h4>{{ Auth::user()->account->plan->present()->promoMessage }}</h4><br/>
  @endif
</div>
...
<script type="text/javascript">
  ...
  function updateUpgradePrices() {
    ...
    @if (Auth::user()->account->plan->hasActivePromo())
        proPrice = proPrice - (proPrice * {{ Auth::user()->account->plan->discount }});
        enterprisePrice = enterprisePrice - (enterprisePrice * {{ Auth::user()->account->plan->discount }});
    @endif
    if (proPrice % 1) {
